...
#shiny dashboard'
RuncharterShiny <- function(data,n = 1,shift_check=FALSE,...){
  ...
  shinyApp(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          sliderInput(inputId = "date", 
                      ...
                      max = as.factor(max(year(data$date))),
                      value = c(2015,2019)),
          
          
          selectInput(inputId = "grp",
                      label = "Indicators",
                      choices = levels(as.factor(data$grp)),
                      #choices = levels(as.factor(trimws(substring(data$grp,1,regexpr("_",data$grp)-1)))),
                      multiple = FALSE,
                      selected = levels(as.factor(data$grp))[2]),
    ...
    server <- function(input, output) {
      ...
      output$plot <- renderPlot({
        ...
        #num <- round(length(levels(as.factor(data$grp)))/2)
      ...
      output$table2 <- renderDataTable({
        ...
        shift_fun <- function(shift_data,run_choice=FALSE){ 
          ...
          shifts<-shift_data$sustained%>%
            ...
            top_n(1,extend_to)#%>%            # To test if this will also select the first record in each group
          ...
          if(run_choice==TRUE){
            ...
            shifts%>%
              ...
              select(`Indicator Name`,`Hospital Site/Business Unit`,`Start of Shift`= start_date,`End of shift` = end_date,`Month since start of shift`= Month_runs)
            ...
            shifts%>%
              ...
              filter(Month_runs<=period_to_check)%>%
              select(`Indicator Name`,`Hospital Site/Business Unit`,`Start of Shift` = start_date,`End of Shift`= end_date,`Month since end of shift` = Month_runs)
        ...
        if (shift_check == TRUE){
          shift_fun(shift_data,run_choice = input$run)
        }
...
RuncharterShiny(data=main_data
                ,shift_check = TRUE
)
